...
const MAX_RETRIES = 3;
const STORAGE_ZONES = process.env.BUNNY_STORAGE_ZONE ? process.env.BUNNY_STORAGE_ZONE.split(",") : ["study-storage-2026"];
...
const uploadChunkToBunny = async (chunk, fileName, folder, chunkIndex, totalChunks, retryCount = 0) => {
    try {
        ...
        if (retryCount < MAX_RETRIES) {
            ...
            return uploadChunkToBunny(chunk, fileName, folder, chunkIndex, totalChunks, retryCount + 1);
        }
...
const uploadLargeFileToBunny = async (fileBuffer, fileName, folder = "videos") => {
    try {
        ...
        for (let i = 0; i < totalChunks; i++) {
            ...
            chunkPromises.push(uploadChunkToBunny(chunk, fileName, folder, i, totalChunks));
        }
